...
`sha256` 不是可选装饰：管理器拒绝安装任何经国内镜像下载、但目录里没有哈希的 Mod；同时它也是判断玩家本地副本是否过期的最可靠依据（作者忘记 bump `version` 时仍然有效）。CI 会比对哈希与磁盘文件，不一致直接失败。

`sha256` is not decoration: the manager refuses mirror-served downloads for entries without one, and uses it to detect stale local copies even when an author forgets to bump `version`. CI compares the hash against the file on disk and fails on mismatch.
