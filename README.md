...
| `sha256` | **Required.** SHA-256 of `file`, generated by `scripts/stamp_hashes.py` | `84c0aa13…c885e0` |
| `size` | **Required.** Byte size of `file`, generated by `scripts/stamp_hashes.py` | `8192` |
| `originUrl` | Optional. Full-size copy when the binary is too large for git (>100 MB). Must be https | `https://github.com/…/releases/download/mods-v1/Big.dll` |
...
### 维护者：改动任何 DLL 后必须补哈希 / Maintainers: re-stamp after touching any DLL

```bash
python scripts/stamp_hashes.py    # 写入/刷新每个条目的 sha256 与 size
python scripts/validate_catalog.py # CI 跑的同一套校验
```

`sha256` 不是可选装饰：管理器对**任何来源**下载的 Mod 都强制校验哈希，缺失即拒绝安装。Mod 是会被加载进游戏进程的 .NET 程序集，镜像和 Release 资源都不具备自证能力。它同时也是判断玩家本地副本是否过期的最可靠依据（作者忘记 bump `version` 时仍然有效）。

`size` 同样必填：下载界面靠它显示进度（续传或经镜像的响应不保证带 `Content-Length`），下载器也靠它在写满磁盘之前就拒掉长度异常的响应。

CI 会把两者都与磁盘文件比对，任一不一致直接失败。

`sha256` is not decoration: the manager verifies **every** download against it regardless of source, and refuses entries that lack one. A mod is a .NET assembly loaded into the game, and neither a mirror nor a release asset is self-authenticating. The hash also detects stale local copies even when an author forgets to bump `version`.

`size` is equally required: the download UI needs a total to show progress against (a resumed or mirror-served response may carry no `Content-Length`), and the downloader uses it to reject an over-long response before it fills the disk.

CI compares both against the file on disk and fails on mismatch.
